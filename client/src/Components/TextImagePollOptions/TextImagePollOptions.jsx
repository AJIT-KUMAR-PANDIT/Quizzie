...
import CssTextImagePollOptions from './TextImagePollOptions.module.css';
...
const TextImageOptions = () => {
  ...
  return (
    <div className={CssTextImagePollOptions.bodyTextImageOptions}>
      <div>
        <div>
          {options.map((option) => (
            <div key={option.id} style={{ display: 'flex' }}>
              <input
                ...
                className={`${CssTextImagePollOptions.radio1} ${option.selected ? CssTextImagePollOptions.selected : ''}`}
                id={`radio${option.id}`}
              />
              <input
                ...
                className={`${CssTextImagePollOptions.inputOption} ${option.selected ? CssTextImagePollOptions.selected : ''}`}
                value={option.text}
              ...
              {options.length > 2 && option.id > 2 && (
                <div>
                  <span
                    className={`${CssTextImagePollOptions.crossIcon} ${CssTextImagePollOptions.crossButton}`}
        ...
        <div className={CssTextImagePollOptions.addButtonContainer}>
          &nbsp;&nbsp;
          {options.length < 4 && (
            <button onClick={handleAddOptionsClick} className={CssTextImagePollOptions.addButton}>Add option</button>
          )}
        </div>
      </div>
    </div>
